...
import { MOCK_APRS, APP_ROUTES } from '../constants';
...
const APRDetails: React.FC = () => {
  ...
  const apr = MOCK_APRS.find(a => a.id === id);

  if (!apr) {
    return (
      <div className="p-8 text-center">
        <h2 className="text-xl font-bold text-gray-800">APR não encontrada</h2>
        <button onClick={() => navigate(APP_ROUTES.APR_LIST)} className="mt-4 text-blue-600 underline">Voltar para a lista</button>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-3">
           <button 
            ...
            <div>
                <h2 className="text-xl font-bold text-gray-900 leading-tight">APR #{apr.id} - {apr.taskName}</h2>
