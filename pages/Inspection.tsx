...
import { APP_ROUTES, CURRENT_USER, MOCK_INSPECTION_TEMPLATE } from '../constants';
import { IconCamera, IconCheckCircle, IconAlertTriangle, IconX, IconSave, IconArrowLeft, IconTrash } from '../components/Icons';
import { createInspection } from '../services/inspectionService';
import { Inspection as InspectionType, InspectionItem, RiskLevel } from '../types';
...
const Inspection: React.FC = () => {
  ...
  useEffect(() => {
    if (!projectId) {
      alert("Erro: Nenhuma obra selecionada para inspeção.");
  ...
  const [formData, setFormData] = useState({
    location: '', // Will be filled by user or fetched
  ...
  const handleFinish = async () => {
    ...
    const newInspection: InspectionType = {
      ...
      projectId: projectId, // Add project ID linkage
    ...
    try {
      ...
      if (result.success) {
        ...
        // Redirect back to project details if projectId exists, otherwise list
        if (projectId) {
          navigate(APP_ROUTES.PROJECT_DETAILS.replace(':id', projectId));
        } else {
          navigate(APP_ROUTES.INSPECTION_LIST);
        }
      } else {
        alert('Erro ao salvar inspeção: ' + result.error);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="pb-24 md:pb-10 max-w-3xl mx-auto">
      ...
      <div className="space-y-4 px-4 md:px-0">
        {items.map((item) => (
          <div key={item.id} className={`bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border-2 transition-all duration-300 ${item.status === 'C' ? 'border-emerald-500' :
            ...
            <div className="flex gap-3 mb-2">
              <button
                ...
                className={`flex-1 py-4 rounded-2xl text-sm font-bold transition-all transform active:scale-95 shadow-sm ${item.status === 'C' ? 'bg-gradient-emerald-main text-white shadow-lg shadow-emerald-200 dark:shadow-none ring-2 ring-emerald-200 dark:ring-emerald-900 ring-offset-2 dark:ring-offset-slate-900' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 border border-gray-200 dark:border-slate-700'
                  } `}
              >
                Conforme
              </button>
              <button
                ...
                className={`flex-1 py-4 rounded-2xl text-sm font-bold transition-all transform active:scale-95 shadow-sm ${item.status === 'NC' ? 'bg-red-600 text-white shadow-lg shadow-red-200 dark:shadow-none ring-2 ring-red-200 dark:ring-red-900 ring-offset-2 dark:ring-offset-slate-900' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 border border-gray-200 dark:border-slate-700'
                  } `}
              >
                Não Conforme
              </button>
              <button
                ...
                className={`flex-1 py-4 rounded-2xl text-sm font-bold transition-all transform active:scale-95 shadow-sm ${item.status === 'NA' ? 'bg-gray-600 text-white shadow-lg shadow-gray-300 dark:shadow-none' : 'bg-gray-50 dark:bg-slate-800 text-gray-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-700 border border-gray-200 dark:border-slate-700'
                  } `}
              >
                N/A
              </button>
