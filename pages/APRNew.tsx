...
import { RiskControl, APR, APRStatus } from '../types';
import { suggestRisksAndControls } from '../services/geminiService';
import { createAPR } from '../services/aprService';
...
const APRNew: React.FC = () => {
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      const result = await createAPR(newAPR);

      if (result.success) {
        alert("APR Enviada para aprovação!");
        navigate(APP_ROUTES.APR_LIST);
      } else {
        alert("Erro ao salvar APR: " + result.error);
      }
    } catch (error) {
      console.error(error);
      alert("Erro inesperado ao salvar APR.");
    } finally {
      setSaving(false);
    }
  };
